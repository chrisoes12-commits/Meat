...
| # | Type | Description | Status |
|---|------|-------------|--------|
| 6 | Reference | **Brand Guide** — master product reference (42 brands) | ✅ Stages 1–5 complete — production ready |
...
| Stage 4 | Photos and logos — wired into moco-stage3.html. Thumbnails on grid cards, banners in sheet, logos below banner. | ✅ Complete — confirmed working on live Netlify deploy (phone + desktop) |
| Stage 5 | Final polish — iPad fix, card text scrims, banner border-radius, logo sizing, home screen icon | ✅ Complete — 2026-06-30 |

**Current working file:** `moco-stage3.html`
**Live URL:** `https://dashing-puffpuff-04f134.netlify.app` (confirmed working with images, 2026-06-30 — supersedes `peaceful-yeot-f4484f` and `creative-maamoul-e006e1`, both retired)
**GitHub repo:** `https://github.com/chrisoes12-commits/Meat`

### Stage 4 — Completed 2026-06-30

All 42 brands have photos and logos wired into `moco-stage3.html`. Code changes were made in Claude Code. Image files are in the project folder and in GitHub.

**What was built:**
- `brandSlug()` helper: converts brand name → slug (lowercase, spaces→hyphens, apostrophes removed)
- Grid cards: `background-image: url(images/brands/thumbs/[slug]-thumb.jpg)` on `.bc-photo-s3`, category gradient kept as CSS fallback
- Sheet banner: `background: url(images/brands/banners/[slug]-banner.jpg) center/cover no-repeat, [gradient]`
- Sheet logo: `<img class="sheet-logo">` in `.sheet-logo-wrap`, centered below banner, `max-height:56px`, `onerror` graceful hide
- Removed `.bc-photo-s3-name` placeholder text (was behind grid card photos)
- Removed `.sheet-banner-name` watermark text (was over sheet banner photo)

**Logo placement decision (locked):** Sheet only — centered below the banner photo, above brand name. Grid cards show photo only.

**Image file locations:**
- `images/logos/[slug]-logo.png` — 42 PNG logos, transparent background
- `images/brands/banners/[slug]-banner.jpg` — 42 banners, 800×450
- `images/brands/thumbs/[slug]-thumb.jpg` — 42 thumbnails, 400×300

**Known content gaps (logos):**
- **MOR** and **Spring Grove** — no clean standalone logo; placeholder is a styled product photo with logo overlay. Revisit if supplier provides a clean file.

**Barkers Creek exception:** Logo has its own baked-in white rounded-rect card. Rendered as a standard `<img>` — no additional treatment applied.

**Deployment note:** Initial deploy to `creative-maamoul-e006e1` was missing the `/images/` folders (not uploaded), causing all photos/logos to fail silently on phone and desktop alike. Re-deployed correctly to `dashing-puffpuff-04f134` with images included — confirmed working. If a future deploy shows missing images, check the `/images/logos/`, `/images/brands/banners/`, and `/images/brands/thumbs/` folders were actually included in the push before debugging code.

**Slug naming convention:** lowercase, spaces → hyphens, apostrophes removed (e.g. "Pioneer's Cut" → `pioneers-cut`, "Schulz's" → `schulzs`).

---

### Stage 5 — Completed 2026-06-30

Polish pass and team rollout prep. All changes are in `moco-stage3.html`.

**What was built:**

1. **iPad sheet fix** — `#brand-sheet` now has `max-width:580px; left:50%; margin-left:-290px` at `min-width:640px`. Sheet is centered on iPad/desktop instead of stretching full viewport width. No JS changes required — swipe JS only modifies `translateY`.

2. **Banner border-radius** — `.sheet-banner` now has `border-radius:18px 18px 0 0` matching the sheet container. Photo no longer bleeds into the rounded corners.

3. **Grid card text scrim** — `.bc-photo-s3::after` pseudo-element adds `linear-gradient(to top, rgba(0,0,0,0.55) 0%, transparent 55%)` over card photos. Brand names stay legible over any image colour.

4. **Logo max-width tightened** — `.sheet-logo` changed from `max-width:200px` to `max-width:160px`. Prevents very wide logos from dominating the sheet header.

5. **Home screen icon** — `images/apple-touch-icon.png` created (180×180px, gold "M" on `#0F0E0B` background, Georgia Bold). Added to `<head>`: `apple-touch-icon` link + `apple-mobile-web-app-capable`, `apple-mobile-web-app-status-bar-style: black-translucent`, `apple-mobile-web-app-title: Moco Meats`. When saved to iPhone home screen, shows the gold M icon and hides Safari browser chrome.

**Deploy note:** `apple-touch-icon.png` is a new file — must include `images/` folder in Netlify upload or the icon will not appear.

**Device verification — confirmed 2026-06-30:**
- Home screen icon: saved live URL to iPhone home screen, gold "M" icon displays correctly
- Cross-brand spot-check: scrolled and opened sheets for multiple brands across different tiers (Super Premium, Premium, Mid-Range) and categories on iPhone — photos, logos, and layout all rendering correctly
- Both checks done against the live Netlify deploy (`dashing-puffpuff-04f134`), not file:// or local
...
| File | Status | Notes |
|------|--------|-------|
| `moco-stage3.html` | Current build | Stages 1–5 complete. This is the file to work from. |
...
| `/images/logos/*.png` | ✅ In place | 42 logo files, transparent PNG, [slug]-logo.png |
| `/images/brands/banners/*.jpg` | ✅ In place | 42 banner photos, 800×450, [slug]-banner.jpg |
| `/images/brands/thumbs/*.jpg` | ✅ In place | 42 thumbnail photos, 400×300, [slug]-thumb.jpg |
| `/images/apple-touch-icon.png` | ✅ In place | 180×180px home screen icon — gold M on dark background |
...
- Cowhide watermark on Beef species card — revisit Stage 5
