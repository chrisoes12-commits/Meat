...
| Stage 4 | Photos and logos — assets sourced, processed, and packaged in Claude.ai. Ready for Claude Code to wire into HTML/CSS. | 🔄 Assets ready — code changes not yet applied |
...
### Stage 4 — Asset Delivery (completed in Claude.ai, this session)

All 42 brands now have finished logo and photo assets, fully reviewed and approved. Two zip files were produced and need to be unzipped into the project folder before Claude Code begins Stage 4 code changes:

- `moco-brand-logos-final-42.zip` → unzip contents into `/images/logos/`
- `moco-brand-photos-final.zip` → unzip contents into `/images/brands/` (contains `banners/` and `thumbs/` subfolders — keep this structure)

**Logo specs:**
- Format: PNG, transparent background, trimmed tight to artwork (no excess padding)
- Filename pattern: `[brand-slug]-logo.png` (e.g. `pioneers-cut-logo.png`, `royal-logo.png`)
- All 42 verified for legibility against `--bg: #0F0E0B` and `--bg-card: #1A1814`
- Exceptions: **Barkers Creek** ships with its own baked-in white rounded-rect card background (logo design didn't read on dark) — do not apply standard dark-background logo treatment to this one file, it's self-contained
- **Icon XB** and **Senku** logos were brightness-adjusted from their original near-black brand colours for legibility — colours are intentionally lighter than brand-strict values

**Photo specs:**
- Banners: JPEG, fixed `800×450` (16:9), filename pattern `[brand-slug]-banner.jpg` — intended for the bottom sheet photo area
- Thumbnails: JPEG, fixed `400×300` (4:3), filename pattern `[brand-slug]-thumb.jpg` — intended for the brand grid card
- All low-resolution source photos were upscaled (Lanczos resampling + unsharp mask) and centre-cropped to fit the fixed ratio — reviewed and approved at actual display size
- **Logo does NOT overlay the photo banner** — per existing Stage 3 documentation, logo and photo are separate elements; logo placement within the sheet is not yet coded (see Open Items below)

**Known content gaps (placeholders only, not final):**
- **MOR** and **Spring Grove** — no clean standalone logo exists in the source deck; the "logo" file for these two is actually a styled product photo with a logo overlay. Acceptable as a placeholder for now. Revisit if/when a proper logo file becomes available from the supplier.

**Slug naming convention:** lowercase, spaces → hyphens, apostrophes removed (e.g. "Pioneer's Cut" → `pioneers-cut`, "Schulz's" → `schulzs`). Use this convention when wiring `BRANDS[]` keys to file paths.
...
| `/images/logos/*.png` | New, Stage 4 | 42 logo files, see Stage 4 section above for specs |
| `/images/brands/banners/*.jpg` | New, Stage 4 | 42 banner photos, 800×450, for bottom sheet |
| `/images/brands/thumbs/*.jpg` | New, Stage 4 | 42 thumbnail photos, 400×300, for brand grid cards |
...
- **Logo placement within the bottom sheet is undecided** — Stage 4 assets are ready (logo file per brand exists), but where exactly the logo sits in the sheet layout (near brand name header? top corner? own row?) has not been designed or coded. Do not guess — confirm with Chris before implementing. Logo on the brand grid card was discussed as "alongside the photo" but exact placement/sizing also not finalised in code.
